...
import { X, Building2, Search, Home } from 'lucide-react';
import type { AdminApartment } from '../api_calls/admin';

interface ApartmentPickerModalProps {
  isVisible: boolean;
  onClose: () => void;
  onSelect: (apartment: AdminApartment) => void;
  selectedId: string;
  apartments: AdminApartment[];
  loading?: boolean;
}

export default function ApartmentPickerModal({
  isVisible,
  onClose,
  onSelect,
  selectedId,
  ...
  const handleSelect = (apt: AdminApartment) => {
    onSelect(apt);
    setSearch('');
    onClose();
  };
        ...
        >
          ...
          <div className="flex items-center justify-between px-6 py-5 border-b border-gray-100">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-lg font-semibold text-gray-900">Seleccionar Departamento</h3>
                <p className="text-xs text-gray-500 mt-0.5">{apartments.length} unidades disponibles</p>
              </div>
            </div>
            <button
              type="button"
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <div className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 ${
                        ...
                        <Building2 className={`w-5 h-5 ${isSelected ? 'text-indigo-600' : 'text-gray-400'}`} />
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className={`font-semibold text-sm ${isSelected ? 'text-indigo-900' : 'text-gray-900'}`}>
                          Unidad {apt.unit}
                        </p>
                        <p className={`text-xs mt-0.5 ${isSelected ? 'text-indigo-600' : 'text-gray-500'}`}>
                          Piso {apt.floor}
                          {apt.rooms ? ` · ${apt.rooms} amb` : ''}
                          {apt.areaM2 ? ` · ${apt.areaM2} m²` : ''}
                        </p>
                      </div>
                      {isSelected && (
